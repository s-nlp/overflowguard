#!/usr/bin/env python3
"""Collect PRE-GENERATION uncertainty on the PISCO compressed path.

Perplexity, Focus and RAUQ are all evaluated on observed textual prompt tokens
that occur after the final compressed-memory position. They come from ONE
prompt-only decoder forward pass: no answer is generated or appended.

The canonical lm-polygraph estimator implementations are reused by presenting
that input span in the same statistics format normally used for generated
output. Therefore Focus and RAUQ here are explicitly *pre-generation
adaptations* of the original methods.

No answer tokens are generated, appended, or teacher-forced in this collector.

Run with PYTORCH_JIT=0 if lm-polygraph's DeBERTa import crashes in your env.
"""
...
def import_router(script_path):
    spec = importlib.util.spec_from_file_location("og_train_pisco", script_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.PiscoRouter


def patch_pisco_loader_for_eager_attention(PiscoRouter):
    """The provided train_pisco.py ignores **kwargs in _load_model."""
    ...
    def _load_model_eager(self, path, **kwargs):
        ...
        if hasattr(self, "_judge_client"):
            self._judge_client = None

    PiscoRouter._load_model = _load_model_eager
    return PiscoRouter
...
def detect_memory_positions(decoder, prompt_ids, prompt_embeds, atol=1e-6):
    ...
    if base.shape != prompt_embeds.shape:
        raise RuntimeError(
            f"PISCO prompt shape changed after replace_emb: base={tuple(base.shape)}, "
...
def main():
    ...
    ap.add_argument("--model", default="naver/pisco-mistral")
    ...
    PiscoRouter = patch_pisco_loader_for_eager_attention(
        import_router(args.router_script)
    )
    router = PiscoRouter.from_pretrained(args.model, attn_implementation="eager")
    router.park_gpu()
    router.model.eval()
    ...
    print(f"[UE] PISCO model: {args.model}")
    ...
    for pos, r in tqdm(selected_rows, desc="PISCO UE"):
        ...
        if prompt_out.attentions is None:
            raise RuntimeError(
                "PISCO returned no attentions. Ensure eager attention is active."
            )
